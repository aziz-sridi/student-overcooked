    ...
    private void showTaskComposerDialog(@Nullable GroupTask taskToEdit) {
        ...
        if (isIndividualProject || currentMembers.isEmpty()) {
            ...
            assigneeDropdown.setOnItemClickListener((parent, view, position, id) -> {
                android.util.Log.d("TaskAssignment", "Assignee position selected: " + position);
                if (position == 0) {
                    selectedAssigneeId[0] = null;
                    selectedAssigneeName[0] = null;
                    android.util.Log.d("TaskAssignment", "Setting unassigned");
                } else {
                    int memberIndex = position - 1;
                    if (memberIndex < currentMembers.size()) {
                        GroupMember member = currentMembers.get(memberIndex);
                        selectedAssigneeId[0] = member.getUserId();
                        selectedAssigneeName[0] = member.getUserName();
                        android.util.Log.d("TaskAssignment", "Selected: " + selectedAssigneeName[0] + " (ID: " + selectedAssigneeId[0] + ")");
                    } else {
                        android.util.Log.w("TaskAssignment", "Member index out of bounds: " + memberIndex);
                    }
                }
            });
        }
        ...
        dialog.setOnShowListener(d -> {
            ...
            positive.setOnClickListener(v -> {
                ...
                android.util.Log.d("TaskSave", "Title: " + title);
                android.util.Log.d("TaskSave", "Assignee ID: " + selectedAssigneeId[0]);
                android.util.Log.d("TaskSave", "Assignee Name: " + selectedAssigneeName[0]);
                ...
                if (isEditing) {
                    groupRepository.updateGroupTask(
                            ...
                            e -> fragment.requireActivity().runOnUiThread(() -> {
                                ...
                                android.util.Log.e("TaskSave", "Error updating task: " + e.getMessage(), e);
                                Toast.makeText(fragment.requireContext(), R.string.error_occurred, Toast.LENGTH_SHORT).show();
                            })
                    );
                } else {
                    groupRepository.createGroupTask(
                            ...
                            e -> fragment.requireActivity().runOnUiThread(() -> {
                                positive.setEnabled(true);
                                android.util.Log.e("TaskSave", "Error creating task: " + e.getMessage(), e);
                                Toast.makeText(fragment.requireContext(), R.string.error_occurred, Toast.LENGTH_SHORT).show();
                            })
                    );
                }
